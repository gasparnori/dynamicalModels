...
class HoHu(object):
    def __init__(self, params, stim_current):
        ...
        for j in range(0, int(params.samples - 1)):
           ...
           # self.taun[j]=1+6/(1+np.exp((vj + 53) / 16))
            ...
            K1v = (-gNa*np.power(mj,3)*hj *(vj-ENa) -gK*np.power(nj, 4)*(vj-EK) -gL*(vj-EL)+stim_current[j])/cm
            ...
            K2v=(-gNa*np.power(am,3)*ah*(av-ENa)-gK*np.power(an,4)*(av-EK)-gL*(av-EL)+stim_current[j+1])/cm
